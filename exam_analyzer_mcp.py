...
def _detect_concepts(text: str) -> List[str]:
    ...
    detected = [concept for concept, kws in concept_patterns.items() if any(kw in text_lower for kw in kws)]
...
def _classify_question(question_text: str, provided_concepts: List[str]) -> dict:
    ...
    return {
        ...
        "has_diagram": any(w in q_lower for w in ("figure", "diagram", "image")),
